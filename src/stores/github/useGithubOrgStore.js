        ...
        const detailedOrgs = await Promise.all(
          response.data.map(async org => {
            try {
              ...
              const orgDetailResponse = await axios.get(`https://api.github.com/orgs/${org.login}`, {
                headers: {
                  Authorization: `Bearer ${authStore.accessToken}`,
                  Accept: "application/vnd.github.v3+json",
                },
              ...
              const membershipResponse = await axios.get(`https://api.github.com/user/memberships/orgs/${org.login}`, {
                headers: {
                  Authorization: `Bearer ${authStore.accessToken}`,
                  Accept: "application/vnd.github.v3+json",
                },
              ...
              const installationResponse = await axios.get(`/api/github/installation-status/${org.login}`, {
                headers: {
                  Authorization: `Bearer ${authStore.accessToken}`,
                },
              });
              ...
              return {
                ...
                isInstalled: installationResponse.data.installed,
              };
            } catch (error) {
