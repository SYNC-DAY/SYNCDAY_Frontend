...
const transformProjectData = projectInfo => ({
	...
	proj_member_id: projectInfo.proj_member_id,
...
export const projectApi = {
	...
	async updateBookmarkStatus(projMemberId) {
		console.log(projMemberId);
		const response = await axios.put(`${BASE_URL}/bookmark?projMemberId=${projMemberId}`);
		if (!response.data.success) {
