<template>
	<div class="container-col">
		<!-- Not Found State -->
		<div
			v-if="!projects[props.projectId]"
			class="flex flex-column items-center justify-center p-6">
		...
		<div
			v-else
			class="proj-container ph-1rem width-100">
			<!-- Header Section -->
			<div class="container-row header underline-gray width-100">
				<div class="header-left container-row">
					<div class="project-title">
						...
						<div v-if="projects[props.projectId].vcs_proj_url">
							<a
								:href="projects[props.projectId].vcs_proj_url"
								target="_blank"
					...
					<div class="container-row role">
						<img
							v-if="projects[props.projectId].participation_status === 'OWNER'"
				...
				<div class="header-right">
					...
					<VcsTypeMenu
						ref="vcsMenu"
						@vcs-selected="handleVcsSelection" />
				</div>
			</div>
			...
			<div class="ph-1rem container-col">
				...
				<!-- Workspaces Grid -->
				<div
					v-if="projects[props.projectId].workspaces?.length"
					class="workspaces-grid">
					<div
						v-for="workspace in projects[props.projectId].workspaces"
						...
						<Card>
							<template #title>
								...
								<Button
									icon="pi pi-code"
									variant="text"
									@click.stop="openRepositoryPopup(workspace)"
									v-tooltip.bottom="'go to repository'" />
							</template>
							<template #content>
								<ProgressBar
									:value="workspace.progress_status"
									severity="secondary" />
							</template>
						</Card>
					</div>
				...
				<div
					v-else
					class="empty-state text-center p-6 bg-gray-50 rounded-lg">
		...
		<Dialog
			...
			<template v-if="selectedWorkspace?.vcs_repo_url">
				<p>Repository URL:</p>
				<a
					:href="selectedWorkspace.vcs_repo_url"
					target="_blank"
					rel="noopener noreferrer"
					@click.prevent="openInPopup"
					class="repo-link">
		...
		<!-- Other Modals -->
		<Dialog
			v-model:visible="openNewWorkspaceDialog"
			modal
			header="새 워크스페이스 생성"
			:style="{ width: '30rem' }">
			<InputText
		...
		<ProjectSettingsModal
			...
			:project-data="projects[props.projectId]"
			@project-updated="handleProjectUpdate"
			@project-deleted="handleProjectDelete" />

		<ProjVcsSettingsModal
			v-model:visible="showProjVcsSettings"
			:project-id="projectId"
			:project-data="projects[props.projectId]"
...
<script setup>
	...
	const openInPopup = () => {
		...
		const popup = window.open(url, "RepositoryWindow", `width=${width},height=${height},left=${left},top=${top},toolbar=no,menubar=no,scrollbars=yes`);
	...
	const createWorkspace = async () => {
		...
		try {
			...
			if (response.data.success) {
				await projectStore.fetchProjects(authStore.user.userId); // Refresh projects data
