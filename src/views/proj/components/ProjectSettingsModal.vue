						...
						<DataTable
							...
							<Column
								...
								<template #body="slotProps">
									...
									<div
										v-if="slotProps.data.participation_status === 'MEMBER'"
										class="status-item">
										<i
											class="pi pi-user"
...
<script setup>
	...
	const isOwner = computed(() => {
		return projectMembers.value.some(member => member.participation_status === "OWNER" && member.user_id === authStore.user.userId);
	});
	...
	const addMember = async selectedUser => {
		try {
			...
			const existingMember = projectMembers.value.find(member => member.user_id === selectedUser.userId);
	...
	const handleSave = async () => {
		try {
			...
			const response = await axios.put(`/proj-members/projs`, {
			...
			if (response.data.success) {
				...
				projectStore.projects[props.projectId] = response.data.data;
